-- Keep NEXT_N recurring rules anchored to the exact occurrence sequence that
-- the member chose.  The materialiser may run later, but it must never replace
-- elapsed or withdrawn dates with later dates.
--
-- This changes future materialisation only. It deliberately does not add,
-- remove, or alter any existing duty_assignments.
...
begin
  ...
  loop
    ...
    -- NEXT_N is anchored to the original start date. The limit is applied to
    -- that complete sequence before passed shifts are excluded. For example,
    -- "next 4" from 13 September remains 13, 20, 27 September and 4 October;
    -- it must not become 20, 27 September and 4, 11 October on 14 September.
    ...
    for v_date in
      ...
      where occurrence.duty_date > v_today
        or (
          occurrence.duty_date = v_today
          and coalesce(v_slot.end_time, time '00:00') > v_now_time
        )
      order by occurrence.duty_date
    loop
