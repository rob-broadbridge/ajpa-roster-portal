-- Stage 19: past-shift registration is an explicitly confirmed, single-slot
-- action. It never creates a recurring rule or an email/calendar notification.
...
begin
  if tg_op = 'INSERT' then
    ...
    -- Do not issue a delayed confirmation or calendar invitation for a shift
    -- that has already finished in the desk's own timezone.
...
begin
  ...
  if v_shift_finished and (p_action <> 'REGISTER' or p_scope <> 'SINGLE') then
    raise exception 'Only a single past shift can be registered. Finished shifts cannot be withdrawn.';
  end if;
