...
Deno.serve(async (request) => {
  ...
  const sendEmail = async (recipient: string, subject: string, text: string, idempotencyKey: string) => {
  ...
  if (type === 'INSERT' && profile && previousProfile === null && profile.slot_id && profile.profile_id && profile.duty_date) {
    ...
    const emailResponse = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { Authorization: `Bearer ${Deno.env.get('RESEND_API_KEY')}`, 'Content-Type': 'application/json', 'Idempotency-Key': `legacy-duty-confirmation-${profile.profile_id}-${profile.slot_id}-${profile.duty_date}` },
      body: JSON.stringify({
  ...
  if (type === 'UPDATE' && previousProfile?.status === 'Pending' && profile.status === 'Approved') {
    ...
    await sendEmail(profile.email, 'AJPA Service Desk Management Platform — access approved', text, `profile-approved-${profile.id}-${profile.updated_at ?? profile.created_at ?? ''}`);
  ...
  await Promise.all(recipients.map((recipient) => sendEmail(recipient, `AJPA Roster Portal: new JP sign-up — ${profile.full_name || profile.email}`, text, `profile-pending-${profile.id}-${profile.created_at ?? ''}-${recipient.toLowerCase()}`)));
